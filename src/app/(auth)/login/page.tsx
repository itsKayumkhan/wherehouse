...
const Page = () => {
  ...
  const [user, setUser] = React.useState<UserT>({ email: "", password: "" });
  ...
  const handelUserChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setUser((prevUser) => ({ ...prevUser, [name]: value }));
  };

  const handelLogin = useGoogleLogin({
  ...
  useEffect(() => {
    if (!token) {
      ...
      if (storedToken) {
        getData(); // Retrieve user details if token exists
      }
    } else {
